...
const translations = {
  ar: {
    ...
    subtitle: 'إدارة حسابات الموظفين وصلاحيات الوصول للنظام.',
    addUserBtn: 'إضافة مستخدم',
    userCol: 'المستخدم',
    salesCol: 'المبيعات والطلبات',
    inventoryCol: 'المخزون والمنتجات',
    purchasesCol: 'المشتريات والموردين',
    invoicesCol: 'الفواتير',
    accountingCol: 'المحاسبة والصناديق',
    actionsCol: 'إجراءات',
  ...
  fr: {
    title: 'Paramètres & Permissions',
    subtitle: 'Gérez les comptes des employés et les accès au système.',
    addUserBtn: 'Ajouter Utilisateur',
    userCol: 'Utilisateur',
    salesCol: 'Ventes & Commandes',
    inventoryCol: 'Stock & Produits',
    purchasesCol: 'Achats & Fournisseurs',
    invoicesCol: 'Facturation',
    accountingCol: 'Compta & Caisses',
    actionsCol: 'Actions',
  ...
  en: {
    ...
    subtitle: 'Manage employee accounts and system access permissions.',
    addUserBtn: 'Add User',
    userCol: 'User',
    salesCol: 'Sales & Orders',
    inventoryCol: 'Inventory & Products',
    purchasesCol: 'Purchases & Suppliers',
    invoicesCol: 'Invoicing',
    accountingCol: 'Accounting & Cashier',
    actionsCol: 'Actions',
...
export default function SupplierTeam() {
  ...
  // حالة نموذج إضافة/تعديل المستخدم
  const [formData, setFormData] = useState({
    id: null,
    full_name: '',
    email: '',
    password: '', // يستخدم فقط عند الإنشاء
    permissions: {
      sales: false,
      inventory: false,
      purchases: false,
  ...
  const handleSaveUser = async (e) => {
    ...
    try {
      if (formData.id) {
        ...
        // إنشاء موظف جديد (يحتاج لإنشاء حساب Auth في الواقع، هنا نسجله في الجدول للتبسيط)
  ...
  const openModal = (user = null) => {
    if (user) {
      setFormData({
        ...
        permissions: user.permissions || { sales: false, inventory: false, purchases: false, invoices: false, accounting: false }
      });
    } else {
      setFormData({
        id: null, full_name: '', email: '', password: '',
        permissions: { sales: true, inventory: false, purchases: false, invoices: false, accounting: false }
      });
    }
    setIsModalOpen(true);
  };

  const PermissionCheck = ({ checked }) => (
    checked ? (
      <div className="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center mx-auto shadow-md shadow-blue-500/20">
        <Check size={14} className="text-white" />
      </div>
    ) : (
      <div className="w-6 h-6 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center mx-auto">
    ...
    <div className="space-y-8 animate-fade-in text-slate-300" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-6 pb-6 border-b border-slate-800">
          ...
          <button onClick={() => openModal()} className="py-2.5 px-5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-500/20 flex items-center gap-2 text-sm cursor-pointer">
          ...
          <table className="w-full text-start">
            <thead>
              <tr className="border-b border-slate-800 text-sm">
                ...
                <th className="pb-4 font-bold text-blue-400 text-center">{t.salesCol}</th>
                <th className="pb-4 font-bold text-emerald-400 text-center">{t.inventoryCol}</th>
                <th className="pb-4 font-bold text-purple-400 text-center">{t.purchasesCol}</th>
                <th className="pb-4 font-bold text-orange-400 text-center">{t.invoicesCol}</th>
                <th className="pb-4 font-bold text-slate-400 text-center">{t.accountingCol}</th>
                <th className="pb-4 font-black text-slate-500 text-center">{t.actionsCol}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {/* المالك (Boss) له كافة الصلاحيات افتراضياً */}
              <tr className="hover:bg-slate-800/30 transition-colors">
                <td className="py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center text-black font-black">
                      {supplier?.store_name?.charAt(0) || 'B'}
                    </div>
                    <div>
                ...
                <td className="py-4 text-center"><PermissionCheck checked={true} /></td>
                <td className="py-4 text-center"><PermissionCheck checked={true} /></td>
                <td className="py-4 text-center"><PermissionCheck checked={true} /></td>
                <td className="py-4 text-center"><PermissionCheck checked={true} /></td>
                <td className="py-4 text-center"><PermissionCheck checked={true} /></td>
              ...
              {isLoading ? (
                <tr><td colSpan="7" className="py-8 text-center"><Loader2 size={30} className="animate-spin text-blue-500 mx-auto"/></td></tr>
              ) : (
                team.map(user => (
                  <tr key={user.id} className="hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="py-4 text-center"><PermissionCheck checked={user.permissions?.sales} /></td>
                    <td className="py-4 text-center"><PermissionCheck checked={user.permissions?.inventory} /></td>
                    <td className="py-4 text-center"><PermissionCheck checked={user.permissions?.purchases} /></td>
                    <td className="py-4 text-center"><PermissionCheck checked={user.permissions?.invoices} /></td>
                    <td className="py-4 text-center"><PermissionCheck checked={user.permissions?.accounting} /></td>
                    <td className="py-4 text-center">
      ...
      {/* 🔐 نافذة الإضافة/التعديل المنبثقة */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 text-start" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden animate-slide-up">
            ...
            <form onSubmit={handleSaveUser} className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {Object.entries({
                    sales: { label: t.salesCol, color: 'blue' },
                    inventory: { label: t.inventoryCol, color: 'emerald' },
                    purchases: { label: t.purchasesCol, color: 'purple' },
                    invoices: { label: t.invoicesCol, color: 'orange' },
                    accounting: { label: t.accountingCol, color: 'slate' }
                  }).map(([key, info]) => (
                    <label key={key} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${formData.permissions[key] ? `bg-${info.color}-500/10 border-${info.color}-500/50` : 'bg-slate-950 border-slate-800 hover:bg-slate-800'}`}>
                      <input type="checkbox" checked={formData.permissions[key]} onChange={() => handleTogglePermission(key)} className="hidden" />
                      <div className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${formData.permissions[key] ? `bg-${info.color}-500 border-${info.color}-500` : 'border-slate-600'}`}>
                        {formData.permissions[key] && <Check size={14} className="text-white" />}
                      </div>
                      <span className={`text-sm font-bold ${formData.permissions[key] ? 'text-white' : 'text-slate-400'}`}>{info.label}</span>
                    </label>
                  ))}
                </div>
              </div>
