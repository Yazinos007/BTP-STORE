...
const translations = {
  ar: {
    title: 'الطلبات اللحظية المباشرة',
    subtitle: 'شاشة الرادار لاستقبال ومعالجة طلبات التجار فور وصولها.',
    ordersCount: 'طلبات',
    noOrders: 'لا توجد طلبات حالياً',
    noOrdersDesc: 'طلبات التزويد الجديدة ستظهر هنا فور وصولها.',
    loadingMerchant: 'جاري التحميل...',
    pending: 'قيد الانتظار',
    confirmed: 'تم التأكيد',
    client: 'العميل (التاجر)',
    totalAmount: 'القيمة الإجمالية',
    currency: 'درهم',
    requestedItems: 'السلع المطلوبة',
    units: 'وحدة',
    gps: 'الموقع',
    rejectTitle: 'رفض الطلب',
    accept: 'قبول الطلب',
    confirmReject: 'هل أنت متأكد من رفض وحذف هذا الطلب؟',
    gpsUnavailable: 'موقع الـ GPS غير متوفر لهذا الطلب',
    error: 'خطأ: '
  },
  fr: {
    title: 'Commandes en Temps Réel',
    subtitle: 'Gérez les demandes de réapprovisionnement des détaillants instantanément.',
    ordersCount: 'Commandes',
    noOrders: 'Aucune commande',
    noOrdersDesc: 'Les nouvelles commandes apparaîtront ici.',
    loadingMerchant: 'Chargement...',
    pending: 'En Attente',
    confirmed: 'Confirmé',
    ...
    accept: 'Accepter',
    confirmReject: 'Refuser et supprimer cette commande ?',
    gpsUnavailable: 'Position GPS non disponible',
    error: 'Erreur: '
  },
  en: {
    title: 'Real-Time Orders',
    subtitle: 'Radar screen to receive and process retailer supply requests.',
    ordersCount: 'Orders',
    noOrders: 'No pending orders',
    noOrdersDesc: 'New supply requests will appear here instantly.',
    ...
    accept: 'Accept Order',
    confirmReject: 'Are you sure you want to reject and delete this order?',
    gpsUnavailable: 'GPS location is unavailable for this order',
    error: 'Error: '
  }
};
    ...
    <div className="space-y-6 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-blue-400">{requests.length} {t.ordersCount}</span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-16 text-center">
          ...
          <p className="text-slate-400">{t.noOrdersDesc}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {requests.map((req) => {
            const isPending = req.status === 'pending';
            const merchantInfo = merchants[req.merchant_id] || { store_name: t.loadingMerchant, phone: '' };
              ...
              <div key={req.id} className={`bg-slate-800/80 backdrop-blur-xl border ${isPending ? 'border-orange-500/30 shadow-[0_0_15px_rgba(249,115,22,0.1)]' : 'border-slate-700'} rounded-2xl overflow-hidden transition-all duration-300 hover:border-slate-600`}>
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <p className="text-sm font-bold text-slate-500 mb-1">{t.client}</p>
                    ...
                    <div className="text-end">
                      <p className="text-sm font-bold text-slate-500 mb-1">{t.totalAmount}</p>
                      <h3 className="text-2xl font-black text-emerald-400" dir="ltr">
                        {Number(req.total_amount).toLocaleString()} <span className="text-sm font-bold opacity-70 uppercase">{t.currency}</span>
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900/50 rounded-xl p-4 mb-6 border border-slate-700/50">
                    <p className="text-xs font-bold text-slate-500 mb-3 uppercase tracking-wider">{t.requestedItems}</p>
                    <ul className="space-y-2">
                      {req.items && req.items.map((item, idx) => (
                        <li key={idx} className="flex justify-between items-center text-sm border-b border-slate-800 pb-2 last:border-0 last:pb-0">
                          ...
                          <span className="text-white font-bold bg-slate-800 px-2 py-1 rounded-md">{item.quantity} {t.units}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="flex gap-3 mt-4">
                    ...
                    {isPending && (
                      <>
                        ...
                        >
                          {processingId === req.id ? <Loader2 size={18} className="animate-spin"/> : <><CheckCircle size={18}/> {t.accept}</>}
                        </button>
                      </>
                    )}
                  </div>
