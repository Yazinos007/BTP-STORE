...
export default function SupplierAccounting() {
  ...
  // مصاريف ثابتة افتراضية (يمكنك لاحقاً ربطها بجدول المصاريف والرواتب)
  const operatingCosts = 399500; 
  const salaries = 62000;
  ...
  const fetchFinancials = async () => {
    setIsLoading(true);
    try {
      // 🌟 جلب المداخيل الحقيقية من الفواتير (Factures)
      ...
      const totalRevenue = data.reduce((sum, doc) => sum + Number(doc.total_amount || 0), 0);
  ...
  // حساب نسب الأعمدة للرسم البياني (بدون مكتبات خارجية)
    ...
    <div className="space-y-6 animate-fade-in text-white" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-purple-500/10 border border-purple-500/20 p-6 rounded-3xl relative overflow-hidden group">
          ...
          <p className="text-purple-400 font-bold text-sm mb-1">{language === 'fr' ? 'Masse Salariale (Mois)' : 'كتلة الأجور (الشهر)'}</p>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-slate-900/50 rounded-xl border border-slate-700/50">
              <span className="font-bold text-slate-300 flex items-center gap-2">
                ...
                {language === 'fr' ? "Produits d'Exploitation (Ventes)" : 'عائدات الاستغلال (المبيعات)'}
              </span>
            ...
            <div className="flex justify-between items-center p-4 bg-slate-900/50 rounded-xl border border-slate-700/50">
              <span className="font-bold text-slate-300 flex items-center gap-2">
                ...
                {language === 'fr' ? "Charges d'Exploitation (Dépenses)" : 'تكاليف الاستغلال (المصاريف)'}
              </span>
            ...
            <div className="flex justify-between items-center p-4 bg-slate-900/50 rounded-xl border border-slate-700/50">
              <span className="font-bold text-slate-300 flex items-center gap-2">
                ...
                {language === 'fr' ? "Frais de Personnel (Salaires)" : 'تكاليف الموظفين (الرواتب)'}
              </span>
