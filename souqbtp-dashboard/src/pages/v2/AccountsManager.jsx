...
export default function AccountsManager() {
  ...
  const ActionModal = () => {
    ...
    const modalColor = isIncome ? 'emerald' : isTransfer ? 'blue' : 'red';
        ...
        <div className={`${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'} border w-full max-w-md rounded-3xl shadow-2xl overflow-hidden`}>
          <div className={`p-6 border-b ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'} flex justify-between items-center`}>
            <h3 className={`text-lg font-black ${textMain} flex items-center gap-2`}>
              {isIncome ? <ArrowDownRight className={`text-${modalColor}-500`} /> : isTransfer ? <ArrowRightLeft className={`text-${modalColor}-500`} /> : <ArrowUpRight className={`text-${modalColor}-500`} />}
              {isIncome ? t.addIncome : isTransfer ? t.transfer : t.addExpense}
            </h3>
          ...
          <div className="p-6 space-y-4">
            ...
            <button onClick={() => setActiveModal(null)} className={`w-full mt-4 py-4 rounded-xl font-black text-white shadow-lg transition-all hover:-translate-y-1 bg-${modalColor}-600 hover:bg-${modalColor}-500 shadow-${modalColor}-500/30 flex justify-center items-center gap-2`}>
    ...
    <div className={`space-y-8 animate-fade-in max-w-7xl mx-auto pb-24`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {accounts.map(acc => {
          const Icon = acc.icon;
          return (
            <div key={acc.id} className={`${bgCard} border-2 p-6 rounded-[2rem] shadow-sm hover:border-${acc.color}-500/50 transition-colors group cursor-pointer`}>
      ...
      {/* Recent Transactions */}
      <div className={`${bgCard} border-2 rounded-[2.5rem] shadow-sm overflow-hidden flex flex-col`}>
          ...
          <table className="w-full text-sm text-start">
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800/50' : 'divide-slate-100'}`}>
              {transactions.map(trx => (
                <tr key={trx.id} className={`transition-colors ${isDarkMode ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}`}>
