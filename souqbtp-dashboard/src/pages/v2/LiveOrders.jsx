...
import { 
  ...
  AlertCircle, Phone, FileSignature, Truck, ArrowUpRight,
  FileText, ShieldCheck, MapPin, X
} from 'lucide-react';
...
export default function LiveOrders() {
  ...
  // 🚀 حالة التتبع (غرفة العمليات المصغرة)
  ...
  const translations = {
    ar: {
      ...
      trackingRoom: 'غرفة تتبع الطلبية', signedContract: 'العقد موقّع رقمياً', route: 'في الطريق إلى العميل...',
      close: 'إغلاق'
    },
    fr: {
      ...
      trackingRoom: 'Salle de Suivi', signedContract: 'Contrat signé numériquement', route: 'En route vers le client...',
      close: 'Fermer'
    },
    en: {
      ...
      pending: 'Pending Review 🟡', waiting: 'Waiting for Signature 🔵', signed: 'Signed (Ready to Ship) 🟢',
      ...
      trackingRoom: 'Order Tracking Room', signedContract: 'Digitally Signed Contract', route: 'En route to client...',
      close: 'Close'
    }
  };
  ...
  useEffect(() => {
    fetchRequests();
    // 🌟 تفعيل الرادار اللحظي (Real-time)
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase.from('supply_requests').select('*').in('status', ['pending', 'waiting_signature', 'signed']).order('created_at', { ascending: false });
      if (error) throw error;
      
      if (data && data.length > 0) {
        setRequests(data);
      } else {
        // 🚀 بيانات وهمية (Fallback) لاستعراض النظام وجمالية الواجهة إذا كانت القاعدة فارغة
        setRequests([
          { id: 'req-1', status: 'pending', total_amount: 45000, created_at: new Date().toISOString(), items: [{name: 'إسمنت بورتلاند', quantity: 50}], merchant_id: 'm1' },
          { id: 'req-2', status: 'waiting_signature', total_amount: 12500, created_at: new Date().toISOString(), items: [{name: 'حديد تسليح 12mm', quantity: 200}], merchant_id: 'm2' },
          { id: 'req-3', status: 'signed', total_amount: 85000, created_at: new Date().toISOString(), items: [{name: 'زليج إسباني (متر)', quantity: 600}], merchant_id: 'm3' },
        ]);
        setMerchants({
  ...
  // 🚀 الموافقة المبدئية (تحول الطلب إلى انتظار التوقيع)
  const handleApproveAndSendContract = async (id) => {
    setProcessingId(id);
    try {
      await supabase.from('supply_requests').update({ status: 'waiting_signature' }).eq('id', id);
      setTimeout(() => {
  ...
  // 🚀 تتبع الـ GPS في النافذة المنبثقة
  ...
  const openGoogleMaps = () => window.open(`https://www.google.com/maps`, '_blank');
  ...
  return (
    <div className={`space-y-8 animate-fade-in max-w-7xl mx-auto pb-24`} dir={isRtl ? 'rtl' : 'ltr'}>
              ...
              <div key={req.id} className={`${cardBg} backdrop-blur-xl border-2 rounded-[2rem] overflow-hidden transition-all duration-300 hover:shadow-xl group`}>
                ...
                <div className={`p-5 border-b ${isDarkMode ? 'border-slate-700 bg-slate-900/40' : 'border-slate-200 bg-white/50'} flex justify-between items-center`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className={`font-black text-lg ${textMain}`}>REQ #{req.id.split('-')[0].toUpperCase()}</h4>
                      <p className={`text-[10px] font-bold ${textMuted} uppercase tracking-widest`}>
                        {new Intl.DateTimeFormat('en-US', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(req.created_at))}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-6">
                  ...
                  {/* 🚀 الأزرار الذكية حسب حالة الطلب */}
                  <div className="flex gap-3 mt-4">
                    {/* زر الخريطة ثابت دائماً */}
                    <button onClick={openGoogleMaps} className={`flex-1 py-3.5 rounded-xl font-black flex justify-center items-center gap-2 transition-all border-2 ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-white' : 'bg-white hover:bg-slate-50 border-slate-200 text-slate-800'}`}>
                    ...
                    {req.status === 'pending' && (
                      <>
                        <button onClick={() => handleRejectOrder(req.id)} className={`px-4 py-3 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 border border-red-500/20 rounded-xl font-bold transition-all`} title={t.rejectTitle}><XCircle size={20}/></button>
      ...
      {/* ========================================================================= */}
      {/* 🚀 غرفة تتبع الطلبية (تظهر فقط للطلبات الموقعة) 🚀 */}
      {/* ========================================================================= */}
        ...
        <div className={`fixed inset-0 z-[9999] ${modalBg} backdrop-blur-xl flex justify-center items-center p-4 animate-fade-in font-cairo`} dir={isRtl ? 'rtl' : 'ltr'}>
          <div className={`${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'} border-2 w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden relative`}>
            ...
            <div className={`p-5 border-b flex justify-between items-center ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-emerald-500/20 text-emerald-500 rounded-xl"><MapPin size={22} className="animate-bounce"/></div>
                <div><h3 className={`text-xl font-black ${textMain}`}>{t.trackingRoom}</h3><p className={`text-xs font-bold ${textMuted}`}>REQ #{trackingOrder.id.split('-')[0].toUpperCase()}</p></div>
              </div>
              <button onClick={() => setTrackingOrder(null)} className={`p-2 rounded-xl transition-colors ${isDarkMode ? 'bg-slate-800 hover:bg-red-500 text-slate-400 hover:text-white' : 'bg-slate-200 hover:bg-red-500 text-slate-600 hover:text-white'}`}><X size={20} /></button>
            </div>

            <div className="p-8 space-y-6">
              {/* شريط الأمان (العقد الموقع) */}
              ...
              <div className={`${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'} border-2 rounded-2xl p-6 relative overflow-hidden shadow-inner`}>
                ...
                <div className={`relative py-6 px-4 my-2 rounded-xl border ${isDarkMode ? 'bg-slate-900/60 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                  ...
                  <div className="absolute top-1/2 -translate-y-1/2 transition-all duration-1000 -ml-4" style={{ [isRtl ? 'right' : 'left']: `${truckProgress}%` }}>
                    <div className="p-2.5 bg-blue-500 text-white rounded-xl shadow-[0_0_20px_#3b82f6] animate-pulse"><Truck size={24} /></div>
                  </div>
