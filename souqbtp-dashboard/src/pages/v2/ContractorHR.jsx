...
import { 
  ...
  CalendarDays, Award, BookOpen, AlertTriangle, Plus, Activity, PhoneCall, HardHat
} from 'lucide-react';
...
export default function ContractorHR() {
  ...
  const [careerEvents, setCareerEvents] = useState([
    { type: 'promotion', title: 'Promotion au poste de Chef de Chantier', date: '2025-11-01' },
  ...
  const translations = {
    ar: {
      ...
      primes: 'المنح / المكافآت', retenues: 'اقتطاعات (CNSS/السلف)',
      ...
      employerSign: 'توقيع المقاولة', employeeSign: 'توقيع الموظف', docInternal: 'مستند داخلي - الأجور',
      profileTooltip: 'فتح ملف الموظف', deleteTooltip: 'حذف الموظف', module: 'إدارة الـ HR للمقاول',
    ...
    fr: {
      title: 'Ressources Humaines', subtitle: 'Gestion de vos employés, équipes de chantier et paie.',
      ...
      employerSign: 'Signature de l\'Entreprise', employeeSign: 'Signature de l\'Employé', docInternal: 'Document Interne - Paie',
      profileTooltip: 'Ouvrir le Profil', deleteTooltip: 'Supprimer', module: 'Module HR Entrepreneur',
    ...
    en: {
      title: 'Human Resources', subtitle: 'Manage your employees, site teams, and payroll.',
      ...
      employerSign: 'Company Signature', employeeSign: 'Employee Signature', docInternal: 'Internal Document - Payroll',
      profileTooltip: 'Open Profile', deleteTooltip: 'Delete', module: 'Contractor HR Module',
  ...
  const openProfile = (emp = null) => {
    if (emp) {
      setFormData({
        ...
        contractType: 'CDI', hireDate: emp.created_at?.split('T')[0] || '2025-01-01', cnss: '123456789', emergency: '0600000000', familyStatus: 'Marié(e)'
      });
      setEditingId(emp.id);
    } else {
      setFormData({ 
    ...
    <div className="space-y-6 animate-fade-in pb-24 max-w-7xl mx-auto" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 mt-2">
        <div>
          ...
          <p className={`mt-1 font-bold ${textMuted}`}>
           {t.subtitle}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => openProfile()} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 hover:bg-blue-700 transition-all font-black shadow-lg shadow-blue-500/30 hover:-translate-y-1">
        ...
        <div className={`p-5 md:p-6 border-b flex flex-col sm:flex-row justify-between items-center gap-4 ${isDarkMode ? 'border-slate-800 bg-slate-800/50' : 'border-slate-200 bg-slate-50'}`}>
          ...
          <div className="relative w-full md:w-80">
            ...
            <input type="text" placeholder={t.searchPlaceholder} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} 
              className={`w-full py-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-sm font-bold ${isRtl ? 'pr-11 pl-4' : 'pl-11 pr-4'} ${inputBg}`} 
            />
          </div>
        </div>

        <div className="overflow-x-auto custom-scrollbar">
          {loading ? <div className="p-12 text-center flex justify-center"><Loader2 size={40} className="animate-spin text-blue-500"/></div> : filteredEmployees.length === 0 ? <div className={`p-16 text-center font-bold ${textMuted}`}><Users size={48} className="mx-auto mb-4 opacity-20"/> {t.empty}</div> : (
            <table className="w-full text-start border-collapse">
              <thead className={`border-b ${isDarkMode ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'}`}>
                <tr>
                  <th className={`px-6 py-5 text-xs font-black uppercase tracking-wider text-start ${textMuted}`}>{t.name}</th>
                  <th className={`px-6 py-5 text-xs font-black uppercase tracking-wider text-start ${textMuted}`}>{t.position}</th>
                  <th className={`px-6 py-5 text-xs font-black uppercase tracking-wider text-start ${textMuted}`}>{t.salary}</th>
                  <th className={`px-6 py-5 text-xs font-black uppercase tracking-wider text-start ${textMuted}`}>{t.status}</th>
                  <th className={`px-6 py-5 text-xs font-black uppercase tracking-wider text-center ${textMuted}`}>{t.actions}</th>
                </tr>
              </thead>
              <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800' : 'divide-slate-100'}`}>
                {filteredEmployees.map(emp => {
                  ...
                  return (
                    <tr key={emp.id} className={`transition-colors group ${isDarkMode ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg uppercase shrink-0 shadow-inner border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-slate-100 border-slate-200 text-blue-600'}`}>
                            {emp.full_name.slice(0, 2)}
                          </div>
                          <div>
                            <p className={`font-black text-base ${textTitle}`}>{emp.full_name}</p>
                            <p className={`text-xs mt-0.5 font-bold ${textMuted}`}>{emp.cin ? `CIN: ${emp.cin}` : ''} {emp.phone ? `| Tél: ${emp.phone}` : ''}</p>
                          </div>
                        </div>
                      </td>
                      <td className={`px-6 py-4 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{emp.role}</td>
                      <td className="px-6 py-4 font-black font-mono text-blue-500 text-base" dir="ltr">
                        {Number(emp.base_salary).toLocaleString()} <span className="text-[10px] font-bold uppercase text-slate-400">{t.currency}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 flex items-center justify-center gap-2 opacity-100 sm:opacity-50 sm:group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openProfile(emp)} className="px-4 py-2 rounded-xl text-sm font-black flex items-center gap-2 transition-colors bg-blue-500/10 text-blue-600 hover:bg-blue-600 hover:text-white" title={t.profileTooltip}>
                          <User size={16} /> {t.profileTooltip}
                        </button>
                        <button onClick={() => handleDeleteClick(emp.id)} className="p-2 rounded-xl transition-colors bg-red-500/10 text-red-500 hover:bg-red-600 hover:text-white" title={t.deleteTooltip}>
                          <Trash2 size={18} />
                        </button>
                      </td>
                    </tr>
      ...
      {showProfileModal && (
        <div className="fixed inset-0 z-[50] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 sm:p-8 text-start" dir={isRtl ? 'rtl' : 'ltr'}>
          <div className={`border-2 rounded-3xl w-full max-w-6xl shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[85vh] ${modalBg}`}>
            ...
            {/* Navigation Tabs */}
            <div className={`flex overflow-x-auto custom-scrollbar border-b shrink-0 px-6 pt-4 gap-2 ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-white'}`}>
              {[
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-5 py-3 font-black text-sm border-b-2 transition-all whitespace-nowrap disabled:opacity-30 disabled:cursor-not-allowed
                    ${activeTab === tab.id 
                      ? 'border-blue-600 text-blue-600 bg-blue-500/10 rounded-t-xl'
                      : `border-transparent ${isDarkMode ? 'text-slate-500 hover:text-slate-300 hover:bg-slate-800' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'} rounded-t-xl`
                    }`}
                >
            ...
            {/* Tab Contents */}
            <div className={`flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar relative ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}>
              
              {/* TAB 1: Infos de Base */}
              {activeTab === 'info' && (
                <form id="emp-form" onSubmit={handleSaveEmployee} className="space-y-8 animate-fade-in">
                  
                  {/* Section: Informations Professionnelles */}
                  <div>
                    <h4 className="text-sm font-black uppercase tracking-widest mb-4 flex items-center gap-2 text-blue-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                      ...
                      <div>
                        <label className={`block text-xs font-bold mb-2 ${textMuted}`}>{t.contractType}</label>
                        <select value={formData.contractType} onChange={e => setFormData({...formData, contractType: e.target.value})} className={`w-full px-4 py-3 rounded-xl outline-none font-bold transition-all border focus:border-blue-500 appearance-none ${inputBg}`}>
                      ...
                      <div>
                        <label className={`block text-xs font-bold mb-2 ${textMuted}`}>{t.hireDate}</label>
                        <input type="date" value={formData.hireDate} onChange={e => setFormData({...formData, hireDate: e.target.value})} className={`w-full px-4 py-3 rounded-xl outline-none font-bold transition-all border focus:border-blue-500 ${inputBg}`} />
                      </div>
                      <div>
                  ...
                  <div>
                    <h4 className="text-sm font-black uppercase tracking-widest mb-4 flex items-center gap-2 text-pink-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                      ...
                      <div>
                        ...
                        <input type="text" value={formData.cnss} onChange={e => setFormData({...formData, cnss: e.target.value})} placeholder="Ex: 123456789" className={`w-full px-4 py-3 rounded-xl outline-none font-bold transition-all border focus:border-pink-500 ${inputBg}`} />
                      </div>
                      <div>
                        <label className={`block text-xs font-bold mb-2 ${textMuted}`}>{t.familyStatus}</label>
                        <select value={formData.familyStatus} onChange={e => setFormData({...formData, familyStatus: e.target.value})} className={`w-full px-4 py-3 rounded-xl outline-none font-bold transition-all border focus:border-pink-500 appearance-none ${inputBg}`}>
                      ...
                      <div className="md:col-span-2">
                        ...
                        <input type="text" value={formData.emergency} onChange={e => setFormData({...formData, emergency: e.target.value})} placeholder="..." className={`w-full px-4 py-3 rounded-xl outline-none font-bold transition-all bg-red-500/5 border border-red-500/20 focus:border-red-500 ${isDarkMode ? 'text-white' : 'text-slate-800'}`} />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Status */}
                  <div>
                    <label className={`block text-xs font-bold mb-2 ${textMuted}`}>{t.status}</label>
                    <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className={`w-full md:w-1/3 px-4 py-3 rounded-xl outline-none font-bold transition-all border focus:border-blue-500 appearance-none ${inputBg}`}>
              ...
              {activeTab === 'paie' && (
                <div className="space-y-8 animate-fade-in">
                  ...
                  <div className={`flex flex-col sm:flex-row items-center justify-between p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                    <div>
                      <p className={`text-sm font-black uppercase tracking-widest ${textMuted}`}>{t.netToPay}</p>
                      <h4 className={`text-3xl font-black mt-1 font-mono ${textTitle}`} dir="ltr">
                    ...
                    <button onClick={() => setShowPayslip(true)} className="mt-4 sm:mt-0 px-6 py-3.5 bg-blue-600 text-white rounded-xl font-black flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20">
                      <FileText size={18}/> {t.payslip} <ChevronRight size={16} className={isRtl ? 'rotate-180' : ''}/>
                    </button>
                  </div>
                </div>
              )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      <h4 className={`font-black text-lg flex items-center gap-2 ${textTitle}`}>
                        <CalendarDays size={20} className="text-orange-500"/> {t.leavesTitle}
                      </h4>
                      <span className="px-4 py-1.5 rounded-lg font-black text-sm border bg-orange-500/10 text-orange-600 border-orange-500/20">
                        {t.leaveBalance} 18 {t.days}
                      </span>
                    </div>

                    <div className={`border rounded-2xl p-4 space-y-4 ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                      {leavesHistory.map((leave, idx) => (
                        <div key={idx} className={`p-4 rounded-xl border flex justify-between items-center ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200 shadow-sm'}`}>
                          <div>
                            <p className={`font-bold ${textTitle}`}>{leave.type === 'Congé Annuel' ? t.annualLeave : leave.type}</p>
                            <p className={`text-xs mt-1 ${textMuted}`}>{leave.start} ➔ {leave.end}</p>
                          </div>
                      ...
                      {showAddLeave ? (
                        <div className="p-4 rounded-xl border mt-4 bg-orange-500/5 border-orange-500/30">
                          <div className="space-y-3">
                            <select className={`w-full px-3 py-2 rounded-lg text-sm font-bold outline-none border appearance-none ${inputBg}`}>
                            ...
                            <div className="flex gap-2">
                              <input type="date" className={`flex-1 px-3 py-2 rounded-lg text-sm outline-none border ${inputBg}`} />
                              <input type="date" className={`flex-1 px-3 py-2 rounded-lg text-sm outline-none border ${inputBg}`} />
                            </div>
                            <div className="flex gap-2 pt-2">
                              <button onClick={() => setShowAddLeave(false)} className={`flex-1 py-2 text-sm font-bold rounded-lg ${isDarkMode ? 'bg-slate-800 text-white hover:bg-slate-700' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>{t.cancel}</button>
                              <button className="flex-[2] py-2 bg-orange-500 text-white text-sm font-black rounded-lg shadow-md hover:bg-orange-600">{t.okBtn}</button>
                            </div>
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className={`border rounded-2xl p-6 ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                      <div className="relative border-l-2 border-slate-300 dark:border-slate-700/50 pl-6 space-y-6 ml-3">
                        ...
                        {careerEvents.map((ev, idx) => {
                          ...
                          return (
                            <div key={idx} className="relative group">
                              <div className={`absolute -left-[35px] w-8 h-8 rounded-full border-4 ${isDarkMode ? 'border-slate-900' : 'border-white'} flex items-center justify-center ${color}`}>
                                <Icon size={12} className="text-current" />
                              </div>
                              <div className={`p-4 rounded-xl border flex justify-between items-start shadow-sm ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
                                <div>
                                  <p className={`font-bold text-sm ${textTitle}`}>{ev.title}</p>
                                  <p className={`text-xs mt-1 ${textMuted}`}>{ev.date}</p>
                                </div>
                                <button onClick={() => setCareerEvents(careerEvents.filter((_, i) => i !== idx))} className="text-red-500 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity p-1">
                                  <Trash2 size={16} />
                                </button>
                              </div>
                            </div>
                        ...
                        {showAddEvent ? (
                          <div className="p-4 rounded-xl border mt-4 bg-purple-500/5 border-purple-500/30">
                            <div className="space-y-3">
                              <select className={`w-full px-3 py-2 rounded-lg text-sm font-bold outline-none border appearance-none ${inputBg}`}>
                              ...
                              <input type="text" placeholder={t.eventTitle} className={`w-full px-3 py-2 rounded-lg text-sm outline-none border ${inputBg}`} />
                              <div className="flex gap-2 pt-2">
                                <button onClick={() => setShowAddEvent(false)} className={`flex-1 py-2 text-sm font-bold rounded-lg ${isDarkMode ? 'bg-slate-800 text-white hover:bg-slate-700' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>{t.cancel}</button>
                                <button className="flex-[2] py-2 bg-purple-500 text-white text-sm font-black rounded-lg shadow-md hover:bg-purple-600">{t.okBtn}</button>
                              </div>
                            </div>
                          </div>
                        ) : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
                  ...
                  <div className="lg:col-span-1 space-y-6">
                    <div className={`p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                      <h4 className={`text-sm font-black uppercase tracking-wider mb-4 flex items-center gap-2 ${textMuted}`}>
                        <Clock size={16} /> {t.historySeniority}
                      </h4>
                      <div className="space-y-4">
                        <div>
                          <p className={`text-xs font-bold mb-1 ${textMuted}`}>{t.hireDate}</p>
                          <p className={`font-black ${textTitle}`}>
                            {formData.hireDate ? new Date(formData.hireDate).toLocaleDateString(language === 'fr' ? 'fr-FR' : language === 'en' ? 'en-US' : 'ar-MA') : '---'}
                          </p>
                        </div>
                        <div>
                          <p className={`text-xs font-bold mb-1 ${textMuted}`}>{t.seniority}</p>
                    ...
                    <div className="p-6 rounded-2xl border bg-blue-500/10 border-blue-500/20">
                      <h4 className="text-sm font-black uppercase tracking-wider mb-4 flex items-center gap-2 text-blue-600">
                        ...
                        {trainings.map((training, idx) => (
                          <li key={idx} className="flex items-start justify-between gap-2 group">
                            <div className="flex items-start gap-2">
                              ...
                              <span className={`text-sm font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{training}</span>
                            </div>
                            <button onClick={() => setTrainings(trainings.filter((_, i) => i !== idx))} className="text-red-500 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity">
                              <Trash2 size={14}/>
                            </button>
                          </li>
                        ))}
                      ...
                      {showAddTraining ? (
                        <div className="mt-4 flex gap-2">
                          <input type="text" value={newTraining} onChange={(e) => setNewTraining(e.target.value)} placeholder={t.trainingName} autoFocus className={`flex-1 px-3 py-1.5 text-sm rounded-lg outline-none border ${inputBg}`} />
                        ...
                        <button onClick={() => setShowAddTraining(true)} className="mt-4 w-full py-2 text-xs font-bold text-blue-600 bg-blue-500/10 hover:bg-blue-500/20 rounded-lg transition-colors">
                          {t.addTraining}
                        </button>
                      )}
                    </div>
                  ...
                  <div className="lg:col-span-2">
                    <div className={`border rounded-2xl p-6 h-full ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                      <h4 className={`font-black text-lg flex items-center gap-2 mb-6 ${textTitle}`}>
                        <FolderOpen size={20} className="text-pink-500"/> {t.legalDocs}
                      </h4>
                      
                      <label className={`block border-2 border-dashed rounded-xl p-8 text-center mb-6 transition-all cursor-pointer relative overflow-hidden ${isDarkMode ? 'border-slate-700 hover:border-pink-500/50 hover:bg-pink-500/5' : 'border-slate-300 hover:border-pink-500/50 hover:bg-pink-50'}`}>
                        ...
                        {isUploading ? (
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className={`font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{t.uploading}</p>
                          </div>
                        ) : (
                          <>
                            <div className={`w-12 h-12 mx-auto rounded-full flex items-center justify-center mb-3 ${isDarkMode ? 'bg-slate-900 text-slate-400' : 'bg-white text-slate-400 shadow-sm'}`}>
                              <Plus size={24} />
                            </div>
                            <p className={`font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{t.dragDropFiles}</p>
                            <p className={`text-xs mt-1 ${textMuted}`}>{t.fileFormats}</p>
                          </>
                        )}
                      </label>
                      ...
                      <div className="space-y-3">
                        {documents.length === 0 ? (
                          <p className={`text-center text-sm py-4 ${textMuted}`}>{t.noDocs}</p>
                        ) : (
                          documents.map(doc => {
                            const DocIcon = doc.icon;
                            return (
                              <div key={doc.id} className={`flex items-center justify-between p-4 rounded-xl border transition-colors hover:border-pink-500/30 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200 shadow-sm'}`}>
                                <div className="flex items-center gap-3">
                                  <div className={`p-2 rounded-lg ${doc.bg} ${doc.color}`}><DocIcon size={18} /></div>
                                  <div>
                                    <p className={`font-bold text-sm ${textTitle}`}>{doc.name}</p>
                                    <p className={`text-xs ${textMuted}`}>{t.addedOn} {doc.date}</p>
                                  </div>
                                </div>
            ...
            {/* Modal Footer */}
            <div className={`p-6 border-t shrink-0 flex gap-3 ${isDarkMode ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'}`}>
              <button onClick={closeProfile} className={`flex-1 py-3.5 rounded-xl font-black transition-colors ${isDarkMode ? 'bg-slate-800 text-white hover:bg-slate-700' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>
                {t.cancel}
              </button>
              {(activeTab === 'info' || activeTab === 'paie') && (
                <button type="submit" form="emp-form" className="flex-[2] py-3.5 bg-blue-600 text-white rounded-xl font-black hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30 hover:-translate-y-1">
                  {t.save}
                </button>
              )}
            </div>
      ...
      {showPayslip && editingId && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[60] flex justify-center items-center overflow-y-auto py-10 animate-fade-in" dir={isRtl ? 'rtl' : 'ltr'}>
          <div className="bg-white text-slate-800 w-[700px] shadow-2xl p-10 relative print-area rounded-2xl animate-slide-up">
            <div className="absolute top-4 right-4 flex gap-2 no-print" dir="ltr">
              <button onClick={() => setShowPayslip(false)} className="p-2.5 bg-slate-100 hover:bg-red-100 text-slate-700 hover:text-red-600 rounded-xl transition-colors font-bold"><X size={20} /></button>
            ...
            <div className="border-2 border-slate-800 p-8 mt-4 rounded-xl">
              <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-6">
                <div>
                  <h1 className="text-3xl font-black text-slate-900 mb-1 uppercase">ENTREPRISE</h1>
                  <p className="text-sm text-slate-500 font-mono">{t.docInternal}</p>
                </div>
                <div className="text-end">
                  <h2 className="text-2xl font-bold text-slate-800 uppercase tracking-widest">{t.payslip}</h2>
                  <p className="text-slate-600 mt-2 font-mono bg-slate-100 px-3 py-1 rounded-lg inline-block">{t.month}: <strong>{currentMonth}</strong></p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-8 mb-8">
                <div><p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-bold">{t.name}</p><p className="text-lg font-black text-slate-900">{formData.full_name}</p></div>
                <div><p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-bold">{t.position}</p><p className="text-lg font-black text-slate-900">{formData.position}</p></div>
                <div><p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-bold">{t.cin}</p><p className="text-base font-bold text-slate-800">{formData.cin || '---'}</p></div>
                <div><p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-bold">{t.cnss}</p><p className="text-base font-bold text-slate-800">{formData.cnss || '---'}</p></div>
              </div>

              <table className="w-full text-start mb-8 border-collapse border border-slate-300">
                <thead>
                  <tr className="bg-slate-100">
                    <th className="border border-slate-300 p-3 font-bold text-slate-700 text-start">{t.designation}</th>
                    <th className="border border-slate-300 p-3 font-bold text-slate-700 text-center">{t.amount}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-slate-300 p-3 font-bold text-slate-800">{t.salary}</td>
                    <td className="border border-slate-300 p-3 text-center font-mono font-bold" dir="ltr">{Number(formData.salary).toLocaleString()} {t.currency}</td>
                  </tr>
                  <tr>
                    <td className="border border-slate-300 p-3 text-slate-600 font-medium">{t.primes}</td>
                    <td className="border border-slate-300 p-3 text-center font-mono text-green-600 font-bold" dir="ltr">+{Number(formData.primes || 0).toLocaleString()} {t.currency}</td>
                  </tr>
                  <tr>
                    <td className="border border-slate-300 p-3 text-slate-600 font-medium">{t.retenues}</td>
                    <td className="border border-slate-300 p-3 text-center font-mono text-red-600 font-bold" dir="ltr">-{Number(formData.retenues || 0).toLocaleString()} {t.currency}</td>
                  </tr>
                </tbody>
              </table>

              <div className="flex justify-end">
                <div className="w-80 bg-slate-50 border-2 border-slate-800 p-5 rounded-xl text-center shadow-inner">
                  <span className="block text-sm text-slate-500 uppercase tracking-widest mb-2 font-bold">{t.netToPay}</span>
                  <span className="text-3xl font-black text-slate-900 font-mono" dir="ltr">
              ...
              <div className="mt-12 pt-6 border-t-2 border-dashed border-slate-200 flex justify-between text-sm font-bold text-slate-500">
      ...
      <style>{`
        @media print {
          ...
          .print-area { position: absolute; left: 0; top: 0; width: 100%; border: none !important; box-shadow: none !important; padding: 0 !important; margin: 0 !important; }
