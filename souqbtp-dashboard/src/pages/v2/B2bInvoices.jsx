...
export default function B2bInvoices() {
  ...
  const translations = {
    ar: {
      title: 'إدارة المستندات والفواتير', subtitle: 'الأرشيف الرسمي لإنشاء وإدارة المبيعات والمشتريات.',
      searchPlaceholder: 'رقم المستند أو التاجر...', filterAll: 'الكل',
      ref: 'المرجع', client: 'العميل / المورد', date: 'التاريخ', type: 'النوع', amount: 'المبلغ', actions: 'إجراءات',
      ...
      docTypes: {
        'Facture': 'فاتورة (Facture)', 'Facture Achat': 'فاتورة شراء (Achat)',
        'Devis': 'عرض سعر (Devis)', 'Bon de Livraison': 'سند تسليم (BL)', 
        'Bon de Commande': 'سند طلب (BC)', "Fiche d'Expédition": 'ورقة إرسال (Expédition)',
        'Avoir': 'فاتورة إرجاع (Avoir)'
      }
    },
    fr: {
      title: 'Gestion des Documents', subtitle: 'Archive officielle pour créer et gérer les ventes et achats.',
      searchPlaceholder: 'N° Document ou Client...', filterAll: 'Tous',
      ref: 'Référence', client: 'Client / Fournisseur', date: 'Date', type: 'Type', amount: 'Montant', actions: 'Actions',
      ...
      docTypes: {
        'Facture': 'Facture', 'Facture Achat': 'Facture d\'Achat',
        'Devis': 'Devis', 'Bon de Livraison': 'Bon de Livraison (BL)', 
        'Bon de Commande': 'Bon de Commande (BC)', "Fiche d'Expédition": 'Fiche d\'Expédition',
        'Avoir': 'Avoir'
      }
    },
    en: {
      title: 'Document Management', subtitle: 'Official archive to create and manage sales and purchases.',
      searchPlaceholder: 'Doc No. or Client...', filterAll: 'All',
      ref: 'Reference', client: 'Client / Supplier', date: 'Date', type: 'Type', amount: 'Amount', actions: 'Actions',
      ...
      docTypes: {
        ...
        'Devis': 'Quote (Devis)', 'Bon de Livraison': 'Delivery Note (BL)', 
        'Bon de Commande': 'Purchase Order (BC)', "Fiche d'Expédition": 'Shipping Note',
        'Avoir': 'Credit Note (Avoir)'
      }
    }
  };
  ...
  const fetchInvoices = async () => {
    ...
    setTimeout(() => {
      setInvoices([
        ...
        { id: 3, ref_number: 'BC-2026-012', type: 'Bon de Commande', client_name: 'مورد حديد صلب', total_amount: 85000, created_at: new Date().toISOString(), items: [{name: 'حديد 12mm', quantity: 10, price: 8500}] }
      ]);
      setIsLoading(false);
    }, 800);
  };
  ...
  const getDocBadgeStyle = (type) => {
    switch(type) {
      case 'Facture': return 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20';
      case 'Facture Achat': return 'bg-teal-500/10 text-teal-500 border-teal-500/20';
      case 'Devis': return 'bg-blue-500/10 text-blue-500 border-blue-500/20';
      case 'Bon de Livraison': return 'bg-purple-500/10 text-purple-500 border-purple-500/20';
      case 'Bon de Commande': return 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20';
      case "Fiche d'Expédition": return 'bg-orange-500/10 text-orange-500 border-orange-500/20';
      case 'Avoir': return 'bg-red-500/10 text-red-500 border-red-500/20';
