...
// 🚀 خوارزمية الرسوم المتحركة للأرقام (CountUp Hook)
function useAnimatedNumber(endValue, duration = 1500) {
...
export default function ContractorAccounting() {
  ...
  // 🚀 تطبيق الأنيمشن على الأرقام الحقيقية
  const animatedRevenue = useAnimatedNumber(revenue, 2000);
  const animatedOpsCosts = useAnimatedNumber(operatingCosts, 2000);
  const animatedSalaries = useAnimatedNumber(salaries, 2000);
  const animatedNetResult = animatedRevenue - animatedOpsCosts - animatedSalaries;
  ...
  const fetchFinancials = async () => {
    ...
    try {
      if(supplier?.id) {
          ...
          const { data: documents } = await supabase.from('documents').select('total_amount, type').or(`owner_id.eq.${targetId},supplier_id.eq.${targetId}`);
          if (documents) {
            documents.forEach(doc => {
              if (doc.type === 'Facture' || doc.type === 'Bon de Commande') calculatedRevenue += Number(doc.total_amount || 0);
              else if (doc.type === 'Facture Achat' || doc.type === 'Avoir') calculatedOpsCosts += Math.abs(Number(doc.total_amount || 0));
            });
          }
          ...
          if (expenses) {
            expenses.forEach(exp => {
              if (exp.category === 'salaires' || exp.category === 'salaries' || exp.category === 'hr') calculatedSalaries += Number(exp.amount || 0);
              else calculatedOpsCosts += Number(exp.amount || 0);
            });
          }

          const { data: taxDeclarations } = await supabase.from('fiscal_declarations').select('tva_due, status').eq('supplier_id', targetId);
          if (taxDeclarations) {
            taxDeclarations.forEach(tax => {
              if (tax.status === 'paid' || tax.status === 'pending') calculatedOpsCosts += Number(tax.tva_due || 0);
            });
          }
      ...
      if (calculatedRevenue === 0 && calculatedOpsCosts === 0) {
          calculatedRevenue = 142500; 
          calculatedOpsCosts = 45500; 
          calculatedSalaries = 28000; 
      }
  ...
  // 🚀 تصاعد نسبة الهدف المالي
  const targetPercentage = Math.min(Math.round((revenue / TARGET_REVENUE) * 100), 100);
  const animatedTargetPercentage = useAnimatedNumber(targetPercentage, 2000);
    ...
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto pb-24" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className={`p-6 rounded-[2rem] relative overflow-hidden group shadow-lg ${isDarkMode ? 'bg-emerald-500/10 border-2 border-emerald-500/20' : 'bg-emerald-50 border-2 border-emerald-200'}`}>
          ...
          <h3 className={`text-3xl font-black ${textMain} font-mono`} dir="ltr">{animatedRevenue.toLocaleString()} <span className="text-xs">{t.currency}</span></h3>
        </div>
        ...
        <div className={`p-6 rounded-[2rem] relative overflow-hidden group shadow-lg ${isDarkMode ? 'bg-orange-500/10 border-2 border-orange-500/20' : 'bg-orange-50 border-2 border-orange-200'}`}>
          ...
          <h3 className={`text-3xl font-black ${textMain} font-mono`} dir="ltr">{animatedOpsCosts.toLocaleString()} <span className="text-xs">{t.currency}</span></h3>
        </div>
        ...
        <div className={`p-6 rounded-[2rem] relative overflow-hidden group shadow-lg ${isDarkMode ? 'bg-purple-500/10 border-2 border-purple-500/20' : 'bg-purple-50 border-2 border-purple-200'}`}>
          ...
          <h3 className={`text-3xl font-black ${textMain} font-mono`} dir="ltr">{animatedSalaries.toLocaleString()} <span className="text-xs">{t.currency}</span></h3>
        </div>
        ...
        <div className={`${isProfitable ? 'bg-gradient-to-br from-blue-600 to-indigo-600' : 'bg-gradient-to-br from-red-600 to-rose-600'} p-6 rounded-[2rem] relative overflow-hidden shadow-xl group text-white`}>
          ...
          <h3 className="text-4xl font-black font-mono" dir="ltr">{animatedNetResult.toLocaleString()} <span className="text-sm">{t.currency}</span></h3>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
        ...
        <div className={`${bgMain} border-2 rounded-[2rem] p-8 shadow-xl flex flex-col`}>
          ...
          <div className={`flex-1 flex items-end justify-center gap-16 h-64 border-b ${isDarkMode ? 'border-slate-700' : 'border-slate-300'} pb-4 relative`}>
            ...
            <div className="w-28 bg-gradient-to-t from-emerald-600 to-emerald-400 rounded-t-2xl relative group flex justify-center transition-all duration-[2000ms] ease-out shadow-[0_0_15px_rgba(16,185,129,0.3)]" style={{ height: revenueHeight, minHeight: '10%' }}>
            ...
            <div className="w-28 bg-gradient-to-t from-orange-600 to-orange-400 rounded-t-2xl relative group flex justify-center transition-all duration-[2000ms] ease-out shadow-[0_0_15px_rgba(249,115,22,0.3)]" style={{ height: expensesHeight, minHeight: '10%' }}>
          ...
          <div className={`mt-10 p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
            ...
            <div className={`w-full h-4 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}>
              <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-[2000ms] ease-out" style={{ width: `${targetPercentage}%` }}></div>
            </div>
            <p className={`text-xs font-bold mt-2 text-end ${textMuted}`}>{t.targetAchieved}: <span className="text-blue-500 font-black">{animatedTargetPercentage}%</span></p>
          </div>
        </div>
        ...
        <div className={`${bgMain} border-2 rounded-[2rem] p-8 shadow-xl flex flex-col`}>
          ...
          <div className="space-y-4 flex-1">
            <div className={`flex justify-between items-center p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <span className="font-black text-lg text-emerald-500 font-mono" dir="ltr">{animatedRevenue.toLocaleString()}</span>
            </div>
            ...
            <div className={`flex justify-between items-center p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <span className="font-black text-lg text-orange-500 font-mono" dir="ltr">-{animatedOpsCosts.toLocaleString()}</span>
            </div>
            ...
            <div className={`flex justify-between items-center p-5 rounded-2xl border ${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'}`}>
              ...
              <span className="font-black text-lg text-purple-500 font-mono" dir="ltr">-{animatedSalaries.toLocaleString()}</span>
            </div>
            ...
            <div className={`mt-8 p-6 rounded-2xl border-2 flex justify-between items-center ${isProfitable ? (isDarkMode ? 'bg-blue-600/10 border-blue-500/30' : 'bg-blue-50 border-blue-200') : (isDarkMode ? 'bg-red-600/10 border-red-500/30' : 'bg-red-50 border-red-200')}`}>
              ...
              <span className={`text-3xl font-black font-mono ${isProfitable ? 'text-blue-500' : 'text-red-500'}`} dir="ltr">
                {animatedNetResult.toLocaleString()} <span className="text-sm font-bold opacity-70 uppercase tracking-widest">{t.currency}</span>
              </span>
            </div>
          </div>
        </div>
