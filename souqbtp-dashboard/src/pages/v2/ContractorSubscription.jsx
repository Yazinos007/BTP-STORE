...
export default function ContractorSubscription() {
  ...
  const translations = {
    ar: {
      ...
      modal: {
        vipTitle: 'خدمة كبار العملاء VIP', vipWelcome: 'مرحباً بك في نادي النخبة', vipDesc: 'مدير حسابات شخصي جاهز لخدمتك.',
        waBtn: 'تواصل عبر WhatsApp', payDirectBtn: 'أفضل الدفع مباشرة', paymentTitle: 'إتمام الدفع',
    ...
    fr: {
      ...
      plans: {
        ...
        enterprise: {
          ...
          features: ['Tout de l\'offre Pro ERP', 'Badge d\'Or "Fournisseur Certifié"', 'Radar d\'Appels d\'Offres Live', 'Audit Financier IA', 'Account Manager Dédié', 'Priorité absolue absolue'],
          missing: []
        }
      },
      modal: {
        vipTitle: 'Service Client VIP', vipWelcome: 'Bienvenue dans le club d\'élite', vipDesc: 'Un manager de compte dédié est prêt à vous assister.',
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 animate-fade-in pb-24" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch mt-12">
        ...
        {/* Enterprise Plan */}
        <div className={`bg-gradient-to-br ${isDarkMode ? 'from-slate-900 to-slate-950 border-amber-500/50' : 'from-amber-50 to-white border-amber-300'} border-2 rounded-[2.5rem] p-8 relative shadow-[0_0_40px_rgba(245,158,11,0.15)] group hover:border-amber-400 transition-all duration-500 text-start flex flex-col`}>
          ...
          <button onClick={() => handleUpgradeClick('enterprise')} disabled={isSubmitting} className="w-full py-4 bg-amber-500 hover:bg-amber-400 text-slate-900 font-black rounded-2xl transition-all shadow-lg shadow-amber-500/30 mb-8 relative z-10 flex justify-center items-center gap-2 text-sm uppercase tracking-widest">
