  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex justify-between items-center">
        <div><h2 className="text-3xl font-black text-gray-800">{t.title}</h2><p className="text-gray-500">{t.subtitle}</p></div>
               ...
               filtered.map(s => (
                <tr key={s.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-end">
                    <span className={`px-3 py-1.5 rounded-lg font-black font-mono ${Number(s.total_debt) > 0 ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`} dir="ltr">
            ...
            <form onSubmit={handlePaymentSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input required type="number" min="1" max={paymentSupplier.total_debt} step="0.01" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} className="w-full pr-12 pl-4 py-3 border border-emerald-200 rounded-xl outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 font-black text-emerald-600 transition-all text-xl" autoFocus />
