...
export default function Fiscal({ isWholesaler }) {
  ...
  // 👻 القضاء على الشبح الأبيض: النظام يكتشف تلقائياً أنه في لوحة القيادة الداكنة!
  const isDarkTheme = isWholesaler || supplier?.supplier_type === 'wholesale' || supplier?.role === 'grossiste' || supplier?.role === 'employé';
  ...
  return (
    <div className="space-y-6 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h2 className={`text-3xl font-black tracking-tight flex items-center gap-3 ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>
           <Landmark className={isDarkTheme ? 'text-blue-500' : 'text-blue-600'} size={32} />
           {t.title}
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        {/* 📝 Form Section */}
        <div className={`${isDarkTheme ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'} border rounded-3xl p-6 shadow-xl h-fit relative transition-colors`}>
          ...
          <h3 className={`text-xl font-black mb-6 flex items-center gap-2 pb-4 border-b ${isDarkTheme ? 'text-white border-slate-700' : 'text-gray-800 border-gray-100'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className={`block text-sm font-bold mb-2 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>{t.period}</label>
              <input type="text" list="periods-list" required value={formData.period} onChange={(e) => setFormData({...formData, period: e.target.value})} 
                className={`w-full px-4 py-3 border rounded-xl outline-none transition-all font-medium ${isDarkTheme ? 'bg-slate-900 border-slate-700 text-white focus:border-blue-500 placeholder-slate-600' : 'bg-gray-50 border-gray-300 text-gray-800 focus:border-blue-500 placeholder-gray-400'}`} 
            ...
            <div>
              <label className={`block text-sm font-bold mb-2 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>{t.tvaCollected}</label>
              <input type="number" required min="0" step="0.01" value={formData.tva_collected} onChange={(e) => setFormData({...formData, tva_collected: e.target.value})} 
                className={`w-full px-4 py-3 border rounded-xl outline-none transition-all font-black text-lg ${isDarkTheme ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 focus:border-emerald-500' : 'bg-emerald-50 border-emerald-200 text-emerald-700 focus:border-emerald-500'}`} 
              />
            </div>
            
            <div>
              <label className={`block text-sm font-bold mb-2 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>{t.tvaDeductible}</label>
              <input type="number" required min="0" step="0.01" value={formData.tva_deductible} onChange={(e) => setFormData({...formData, tva_deductible: e.target.value})} 
                className={`w-full px-4 py-3 border rounded-xl outline-none transition-all font-black text-lg ${isDarkTheme ? 'bg-orange-500/10 border-orange-500/30 text-orange-400 focus:border-orange-500' : 'bg-orange-50 border-orange-200 text-orange-700 focus:border-orange-500'}`} 
              />
            </div>
            
            <div className={`p-5 rounded-2xl border flex justify-between items-center ${isDarkTheme ? 'bg-blue-900/20 border-blue-500/20' : 'bg-blue-50 border-blue-100'}`}>
              <span className={`text-sm font-black ${isDarkTheme ? 'text-blue-400' : 'text-blue-800'}`}>{t.tvaDue} :</span>
              <span className={`text-2xl font-black font-mono ${isDarkTheme ? 'text-blue-300' : 'text-blue-700'}`} dir="ltr">{tva_due_calculated.toFixed(2)} <span className="text-sm">{t.currency}</span></span>
            </div>

            <div>
              <label className={`block text-sm font-bold mb-2 ${isDarkTheme ? 'text-slate-300' : 'text-gray-700'}`}>{t.status}</label>
              <select value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})} 
                className={`w-full px-4 py-3 border rounded-xl outline-none transition-all font-bold appearance-none ${isDarkTheme ? 'bg-slate-900 border-slate-700 text-white focus:border-blue-500' : 'bg-white border-gray-300 text-gray-800 focus:border-blue-500'}`}
              >
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-700/50">
              {editingId && (
                <button type="button" onClick={cancelEdit} className={`flex-1 py-3.5 rounded-xl font-black transition-all flex items-center justify-center gap-2 ${isDarkTheme ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  <X size={18}/> {t.cancel}
                </button>
              )}
        ...
        <div className="lg:col-span-2">
          <div className={`${isDarkTheme ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'} border rounded-3xl shadow-xl overflow-hidden h-full flex flex-col`}>
            <div className={`p-5 md:p-6 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isDarkTheme ? 'bg-slate-900/50 border-slate-700' : 'bg-gray-50 border-gray-100'}`}>
              <div className="flex items-center gap-3">
                <FileCheck size={20} className={isDarkTheme ? 'text-slate-400' : 'text-gray-500'} />
                <h3 className={`font-black text-lg ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>{t.history}</h3>
              </div>
            ...
            <div className="flex-1 overflow-x-auto custom-scrollbar">
              {isLoading ? ( 
                ...
                <div className={`flex items-center justify-center h-64 font-bold ${isDarkTheme ? 'text-slate-500' : 'text-gray-400'}`}>{t.empty}</div> 
              ) : (
                <table className="w-full text-start text-sm">
                  <thead className={`border-b ${isDarkTheme ? 'border-slate-700 bg-slate-900/80 text-slate-400' : 'border-gray-100 bg-white text-gray-500'}`}>
                    <tr>
                  ...
                  <tbody className={`divide-y ${isDarkTheme ? 'divide-slate-700/50' : 'divide-gray-50'}`}>
                    {filteredDeclarations.map((dec) => (
                      <tr key={dec.id} className={`transition-colors group ${isDarkTheme ? 'hover:bg-slate-700/30' : 'hover:bg-blue-50/30'} ${editingId === dec.id ? (isDarkTheme ? 'bg-slate-700/50' : 'bg-blue-50') : ''}`}>
                        <td className={`px-6 py-5 font-bold ${isDarkTheme ? 'text-white' : 'text-gray-800'}`}>{dec.period}</td>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex justify-center gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEdit(dec)} className={`p-2 rounded-xl transition-all shadow-sm ${isDarkTheme ? 'bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white'}`}><Edit size={16}/></button>
                            <button onClick={() => handleDelete(dec.id)} className={`p-2 rounded-xl transition-all shadow-sm ${isDarkTheme ? 'bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white' : 'bg-red-50 text-red-600 hover:bg-red-600 hover:text-white'}`}><Trash2 size={16}/></button>
                          </div>
                        </td>
                      </tr>
                    ))}
