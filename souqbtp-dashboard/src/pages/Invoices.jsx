...
import { FileText, Search, Printer, Trash2, HardHat, FileSpreadsheet, File, FileOutput, Loader2 } from 'lucide-react';

const translations = {
  ar: {
    title: 'إدارة المستندات والفواتير', subtitle: 'أرشيف كامل لكل عروض الأسعار، الفواتير، وسندات التسليم.',
    search: 'ابحث برقم المستند أو اسم العميل...', all: 'الكل',
    docNo: 'رقم المستند', client: 'العميل والورش', type: 'النوع', date: 'التاريخ', total: 'المجموع', actions: 'إجراءات',
  ...
  fr: {
    ...
    search: 'Rechercher par N°, Client...', all: 'Tous',
    docNo: 'N° Document', client: 'Client & Chantier', type: 'Type', date: 'Date', total: 'Total', actions: 'Actions',
...
export default function Invoices() {
  ...
  // 🖨️ دالة إعادة الطباعة (نفس التصميم الاحترافي الموجود في POS)
  const handleReprint = (doc) => {
    ...
    const clientName = doc.clients?.full_name || 'Client Inconnu';
            ...
            <tbody>
              ${items.map(item => `
                <tr>
                  ...
                  <td style="text-align:right; font-weight:bold;">${(item.quantity * item.price).toLocaleString()}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* 📑 أزرار التبويب (Tabs) الذكية */}
      <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2">
        {['All', 'Facture', 'Devis', 'Bon de Livraison', 'Bon de Commande'].map(tab => (
          <button 
          ...
          >
            ...
            <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${activeTab === tab ? 'bg-white/20' : 'bg-gray-100 text-gray-400'}`}>
              {tab === 'All' ? safeDocuments.length : safeDocuments.filter(d => d.type === tab).length}
            </span>
          </button>
        ))}
      </div>
              ...
              {isLoading ? (
                ...
                <tr key={doc.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <p className="font-bold text-gray-800">{doc.clients?.full_name || '---'}</p>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => handleDelete(doc.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-lg transition-colors" title="Supprimer"><Trash2 size={16}/></button>
                    </div>
                  </td>
                </tr>
              ))}
