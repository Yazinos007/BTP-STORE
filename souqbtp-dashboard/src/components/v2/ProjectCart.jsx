...
export default function ProjectCart({ isDarkMode, language, onClose }) {
  ...
  const t = {
    ar: {
      ...
      projectName: "فيلا بن علي",
      completion: "نسبة الإنجاز",
      types: {
        product: "مواد بناء",
        service: "خدمات وحرفيين",
        rental: "معدات وآليات"
      },
      status: {
        pendingQuote: "في انتظار عرض السعر",
        available: "متاح للحجز",
        inStock: "متوفر"
      },
      actions: {
        checkout: "تأكيد الطلبات",
        requestQuotes: "طلب عروض الأسعار",
        remove: "إزالة"
      },
      summary: {
    ...
    fr: {
      title: "Panier du Projet",
      activeProject: "Projet Actif :",
      projectName: "Villa Benali",
      completion: "Complétion",
      types: {
        product: "Matériaux",
        service: "Services & Artisans",
        rental: "Équipements"
      },
      status: {
      ...
      actions: {
        checkout: "Valider les commandes",
        requestQuotes: "Demander les devis",
        remove: "Retirer"
      },
      summary: {
  ...
  // بيانات وهمية تمثل المزيج الذكي لسلة BTP
  const cartItems = [
    {
      id: 1,
      type: 'product',
      ...
      currency: 'MAD',
      unit: 'Sacs',
      image: 'https://images.unsplash.com/photo-1589939705384-5185137a7f0f?w=150'
    },
    {
      id: 2,
      type: 'rental',
      name: 'CAT 320 Excavatrice',
      supplier: 'Atlas Engins',
      duration: 3,
      price: 1800,
      currency: 'MAD/jour',
      transport: 500,
      dates: '25-27 Sept',
      image: 'https://images.unsplash.com/photo-1579762699924-a74087cb8916?w=150'
    },
    {
      id: 3,
      type: 'service',
      name: 'Installation Électrique',
      supplier: 'Ahmed Électricité',
      status: 'pending',
      desc: 'Câblage complet RDC',
      image: 'https://images.unsplash.com/photo-1621905252507-b35492cc74b4?w=150'
    }
  ];
  ...
  const suppliersCount = new Set(cartItems.map(i => i.supplier)).size;
  const pendingQuotes = cartItems.filter(i => i.type === 'service').length;
    ...
    >
      ...
      >
        ...
        <div className={`p-6 border-b ${borderStyle} bg-slate-900 text-white relative`}>
          <button 
            onClick={onClose}
            className={`absolute top-6 ${isRtl ? 'left-6' : 'right-6'} p-2 bg-slate-800 hover:bg-red-500 rounded-full transition-colors`}
          >
            <X size={18} />
          </button>
          ...
          {cartItems.map((item) => (
            <div key={item.id} className={`rounded-2xl border ${borderStyle} ${bgCard} overflow-hidden shadow-sm`}>
              ...
              >
                ...
                {item.type === 'service' && <Clock size={14} />}
                {t.types[item.type]}
              </div>
              ...
              <div className="p-4 flex gap-4">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <button className="text-slate-400 hover:text-red-500 transition-colors p-1 -mt-1"><Trash2 size={16} /></button>
                  </div>
                  <p className={`text-xs ${textMuted} mb-3`}>{item.supplier}</p>
                  
                  {/* التسعير والكمية بناءً على نوع المعاملة */}
                  {item.type === 'product' && (
        ...
        <div className={`p-6 border-t ${borderStyle} ${bgCard} shadow-[0_-10px_20px_rgba(0,0,0,0.05)]`}>
          ...
          <div className="space-y-3">
            {pendingQuotes > 0 && (
              <button className={`w-full py-3 rounded-xl font-bold border-2 flex items-center justify-center gap-2 transition-colors ${isDarkMode ? 'border-slate-700 text-white hover:bg-slate-800' : 'border-slate-300 text-slate-800 hover:bg-slate-50'}`}>
                <FileText size={18} /> {t.actions.requestQuotes}
              </button>
            )}
            <button className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3.5 rounded-xl font-black flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 transition-all">
