...
const translations = {
  ...
  en: {
    title: 'SouqBTP Smart Assistant',
    desc: 'Forecasts based on current market price analysis for sector: ',
    premiumBadge: 'PREMIUM',
    lockedTitle: 'Exclusive Subscriber Feature',
    lockedDesc: 'Upgrade your account to unlock AI analytics and price predictions for your products.',
    upgradeBtn: 'Upgrade Account Now',
    ignoreBtn: 'Ignore',
    emptyState: 'No new analytics for your specialty today..',
    categories: {
      gros_oeuvre: 'Steel & Cement',
      plomberie: 'Plumbing & Pipes',
      electricite: 'Electricity & Wires',
...
export default function SmartAIAssistant() {
  ...
  const filteredTrends = trends.filter(tr => tr.category === specialty || tr.category === 'general');
  const displaySpecialty = t.categories[specialty] || specialty;

  return (
    <div className="bg-gradient-to-br from-[#1e293b] to-[#0f172a] rounded-[23px] p-6 sm:p-8 shadow-2xl relative overflow-hidden mb-8 border border-white/5 min-h-[260px] text-start" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-3 mb-8">
          <div className="p-3 bg-blue-500/20 text-blue-400 rounded-2xl shadow-[0_0_15px_rgba(59,130,246,0.2)] shrink-0">
            <BrainCircuit size={28} />
          </div>
          <div>
            <h3 className="text-xl md:text-2xl font-black text-white flex items-center gap-2 flex-wrap">
              {t.title} <Sparkles size={18} className="text-yellow-400" />
              {!hasAccess && <span className="text-[10px] bg-amber-500 text-black px-2 py-0.5 rounded-md font-black tracking-wider">{t.premiumBadge}</span>}
            </h3>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 relative">
            ...
            {!hasAccess && (
              <div className="absolute inset-0 z-20 bg-[#0f172a]/90 backdrop-blur-sm rounded-2xl flex flex-col items-center justify-center p-6 text-center border border-white/10 shadow-2xl">
                ...
                <button className="px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black font-black text-sm rounded-xl transition-transform hover:scale-105 shadow-[0_0_20px_rgba(245,158,11,0.4)] cursor-pointer">
                  {t.upgradeBtn}
                </button>
              </div>
            )}
                ...
                return (
                  <div key={trend.id} className={`bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-colors flex flex-col justify-between group ${!hasAccess ? 'filter blur-[2px] opacity-30 pointer-events-none' : ''}`}>
                    ...
                    <div className="flex items-center gap-3 pt-5 border-t border-white/10 mt-auto">
                      <button className={`flex-1 py-3 px-4 rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all shadow-lg cursor-pointer ${isWarning ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white' : 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white'}`}>
                        {trend.action_text} <ArrowRight size={18} className={language === 'ar' ? 'rotate-180' : ''} />
                      </button>
                      <button onClick={() => dismissInsight(trend.id)} className="px-5 py-3 bg-white/5 hover:bg-white/10 text-gray-300 font-bold text-sm rounded-xl transition-colors cursor-pointer">
                        {t.ignoreBtn}
                      </button>
                    </div>
                  </div>
